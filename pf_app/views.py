...
from pf_app.models import ContactForm,CustomUser
#</------ My Portfolio Website Internal Project Packages -----
...
def user_login(request):
    ...
    if request.method == "POST":
        ...
        if user is not None:
            if user.is_active:
                login(request,user)
                return HttpResponseRedirect(reverse('pf_app:all_data'))
        else:
            return render(request,'pf_app/registration/login.html',{'error_message': 'Username or Password Incorrect!'})
    else:
        return render(request,'pf_app/registration/login.html')
...
def register(request):
    ...
    if request.method == "POST":
        ...
        if password != confirm_password:
            return render(request,'pf_app/registration/register.html',{'error_message':'Password Not Match!'})
        if CustomUser.objects.filter(username=username).exists():
            return render(request,'pf_app/registration/register.html',{'error_message':'Username already exist!'})
        else:
            user = CustomUser.objects.create(username=username, password= make_password(password),role=2)
            login(request,user)
            return HttpResponseRedirect(reverse('pf_app:all_data'))
    else:
        return render(request,'pf_app/registration/register.html')
...
def log_out(request):
    ...
    logout(request)
    return HttpResponseRedirect(reverse('pf_app:login'))
...
def about(request):
    ...
    return: nothing  """
    return render(request,'pf_app/about.html')
...
def portfolio(request):
    ...
    parameters: HttpRequest
    return: nothing  """
    return render(request,'pf_app/portfolio.html')
...
def contact(request):
    ...
    parameters: HttpRequest
    return: nothing  """
    return render(request,'pf_app/contact.html')
...
def contact_form_submit(request):
    ...
    return HttpResponseRedirect(reverse('pf_app:contact'))
...
def all_data(request):
    ...
    return render(request,'pf_app/all_data.html',data)
...
def users(request):
    ...
    data = {'users':users}
    return render(request,'pf_app/users.html',data)
